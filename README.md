...
旧画像しかない間は元の配列を維持するため、商品画像修正前にこのコードが先行しても掲載件数を減らしません。旧画像がまだ先頭で、他画像が後ろにある場合は `MID_AGENT_PRIMARY_PLACEHOLDER` で生成を停止します。副画像を自動的に主画像へ昇格させず、Shopifyでの並べ替え完了を確認してから再生成してください。画像修正を巻き戻した場合にもこの停止条件を確認します。
